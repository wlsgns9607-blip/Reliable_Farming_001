  ...
  const playUsageGuide = () => {
    if ('speechSynthesis' in window) {
      ...
      if (location.pathname === '/ai-expert') {
        text = "어르신, 요 화면은 내 밭에 궁금한 거 물어보는 척척박사 방이여요. 사용하시는 방법 참 쉽습니다! 첫번째로, 저 밑에 마이크 그림 보이시지오? 그거 손가락으로 꾹 누르시고, '올해 고추 심을 때 거름을 얼마나 줘야 하냐~' 하고 편하게 말씀만 하셔요. 두번째로, 그러면 요 박사가 글씨랑 목소리로 싹 대답해 줄 겁니다. 세번째로, 만약 이 대답을 멀리 사는 자녀들한테도 보여주고 싶으시면, 대답 밑에 협업일지에 복사 버튼을 툭 누르셔요. 그러면 우리 자녀들이 보는 소통방에 아버님이 공부하신 내용이 그대로 복사돼서 쏙 들어갑니다! 네번째로, 화면에서 글씨가 길어서 다 안 보일 때는 손가락으로 화면을 위아래로 슥슥 쓸어보셔요. 그러면 예전에 물어봤던 내용도 다 보실 수 있답니다!";
